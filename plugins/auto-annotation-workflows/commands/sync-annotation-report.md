...
Sync the latest auto-annotation artifacts from the remote server, compare them against the manual labels, and write a dated evaluation report.
...
2. Confirm remote artifacts exist on `<host>:~/workspace/auto-annotation-research/new/annotation_tool`:
...
5. Note any uncommitted local changes that could make the report ambiguous, but do not block on them.

## Plan

1. Sync the latest run artifacts from the remote host into `new/annotation_tool/kiwi_logs/`.
...
If CSV and JSONL disagree, use the latest completed run in `inference_process.jsonl` as the evaluation truth and call that out explicitly.
...
- Confirm `new/annotation_tool/annotated_samples_auto.csv` matches the synced CSV unless JSONL must override it.
...
- If the run is invalid or partially reflected, fix the sync/runtime mismatch first.
- If one failure mode dominates, propose the narrowest next implementation target.
- If prompt or modeling changes are proposed, point to the next command:
