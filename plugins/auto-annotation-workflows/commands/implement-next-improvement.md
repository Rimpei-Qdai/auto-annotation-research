...
Implement the next focused improvement based on the latest evaluation evidence, verify it, and commit it.

## Arguments

- `focus`: optional focus area such as `prompt`, `summary`, `graph`, `logging`, `sync`, `runtime`
...
4. If the improvement is primarily a prompt change, first confirm there is literature support and record the basis in a new or updated report.
...
- Preserve current input/output file contracts unless the change requires a migration.
- Do not commit generated CSVs or remote-only artifacts unless the user explicitly asks.
...
Commit meaningful code changes after verification. Keep the commit scoped to the focused improvement.
...
- If the change is ready for evaluation, run `/sync-annotation-report` after the next annotation run.
- If the evidence is still weak or contradictory, use `/investigate-and-propose`.
