...
Render the current annotation inputs for representative samples so the visual
prompt design can be inspected directly.
...
3. Confirm videos and trajectory generation paths exist locally under
   `new/filterd_video/`.
...
4. Show representative output paths and, when helpful, render a few example
   images in chat.
...
Do not generate `contact_sheet.png` unless `save_mode=with-collage` or the user
explicitly asks.
...
- Confirm the saved images match the current code path, not an old overlay
  style.
