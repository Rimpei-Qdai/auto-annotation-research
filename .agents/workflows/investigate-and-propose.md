...
Understand the current failure mode, survey literature, and propose a deeper
improvement plan with clear implementation order.

## Arguments

- `focus`: optional research focus such as `vlm-trajectory-reading`,
  `prompt-routing`, `graph-design`, `summary-design`, `macro-to-fine`
...
3. If the latest remote run has not been synced yet, sync it first from `host`,
   defaulting to `kiwi`.
