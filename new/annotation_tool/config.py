...
SENSOR_TEMPORAL_WINDOW_SECONDS = 3.0
SENSOR_TEMPORAL_FALLBACK_WINDOW_SECONDS = 10.0
VIDEO_CANDIDATE_TOP_K = 5
VIDEO_CHOICE_BONUS = 0.35
MAX_NEW_TOKENS_VIDEO_CANDIDATE = 8
...
# A -> 1 (直進系), B -> 6 (左回転系), C -> 7 (右回転系), D -> 0 (その他)
...
VIDEO_CANDIDATE_SELECTION_PROMPT_TEMPLATE = """あなたは運転行動を分類するAIです。
...
入力には、判定対象時刻を中心とした前後3秒、合計6秒の動画クリップと、軌道・運動を要約した3枚の画像があります。

- 動画: 判定対象時刻の前後3秒を含む raw video clip
- 画像1: fixed-scale の top-down trajectory summary
- 画像2: 左右の曲がりを強調した normalized geometry summary
- 画像3: speed / yaw rate / brake / blinker の時系列を示す motion summary
- 画像1,2 の緑線は直進基準、赤線は今後3秒の予測軌道です
- 画像3 の NOW は判定対象時刻です
- 動画では道路文脈と実際の動きを見てください
- 画像1,2では将来の進行方向と曲がりの強さを見てください
- 画像3では、その前後で速度・回転・ブレーキ・ウィンカーがどう変化したかを見てください
...
- 動画と軌道画像が矛盾するときは、道路文脈は動画、将来の向きは画像1,2、運動の変化は画像3を優先して総合判断してください
